(() => {
  const start = () => {
    ...
    const chooseButton = document.getElementById("project-logo-choose-button");
    const uploadButton = document.getElementById("project-logo-upload-button");
    const progress = document.getElementById("project-logo-progress");
    ...
    const antiForgeryToken = document.querySelector("#project-form input[name='__RequestVerificationToken']");
    const modalElement = document.getElementById("project-logo-editor-modal");
    const stage = document.getElementById("project-logo-crop-stage");
    const canvas = document.getElementById("project-logo-crop-canvas");
    const zoom = document.getElementById("project-logo-zoom");
    const editorError = document.getElementById("project-logo-editor-error");
    ...
    const createLogoBlob = async (maxBytes = 512 * 1024) => {
      if (!image) {
    ...
    const uploadLogo = async () => {
      ...
      setEditorError();
      try {
        const blob = await createLogoBlob();
        const body = new FormData();
        body.append("logo", blob, "logo.jpg");
        if (antiForgeryToken) {
          body.append("__RequestVerificationToken", antiForgeryToken.value);
        }

        const response = await fetch(`${window.location.pathname}?handler=Logo`, {
          method: "POST",
          body
        });
        ...
        const logoUrl = result.displayLogoUrl || result.logoUrl;
        document.querySelectorAll("[data-project-logo]").forEach((logoImage) => {
        ...
        const message = error.message || "Logo upload failed. Please try again.";
